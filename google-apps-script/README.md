...
Google Form을 통해 제출된 성함과 축하 메시지가 blank가 아닌 경우, 자동으로 방명록 스프레드시트에 다음 정보를 업데이트합니다:
- **이름** (forms 성함 → 방명록 From 컬럼)
- **축하 메시지** (forms 축하 메시지 → 방명록 본문 컬럼)
- **제출 시간** (자동 생성)
...
- **축하 메시지** (질문 제목에 "축하"와 "메시지" 또는 "메세지" 포함)
...
1. **필드명 일치**: Form의 질문 제목에 "성함"과 "축하 메시지" (또는 "축하 메세지")가 정확히 포함되어야 합니다
2. **빈 값 처리**: 이름 또는 메시지가 비어있으면 방명록에 추가되지 않습니다
...
2. Form 필드명이 "성함"과 "축하 메시지" (또는 "축하 메세지")를 포함하는지 확인
